...
Table with the available flags:
...
- **Check video data:** doesn't change anything. Shows a searchable list of the videos registered for the chosen preset — typing filters by filename, title and description — and prints everything the tool has stored about the selected one: database ID, title, description, availability, status (stored locally or uploaded), filename, original video URL, publication date and the dates it was created and last updated in the database. Right after, it prints one block per part already uploaded for that video, each with its Telegram message ID. The availability and the dates are shown using the display texts and the date format defined in the preset, and fields with no value are shown as `(empty)`. It finishes by asking `Return to menu?` (the default answer is yes).
...
Once an action finishes, the program ends — unless it sends you back to the action menu. That happens when you decline the `Proceed?` confirmation of the upload action, when you choose the `Cancel edit` option of the edit action, when you decline the deletion confirmation of the delete action, and when you answer yes to the `Return to menu?` question that the preset data and video data actions ask after printing everything (yes is the default). To leave from the menu itself, use the **Exit** option.
