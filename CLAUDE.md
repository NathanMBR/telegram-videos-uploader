...
- **`src/usecases/`** — orchestration. Every usecase is a class extending the `Usecase` abstract class (abstract `preset` field + `async execute()`, plus the inherited `printDryRunMessage()`), takes the chosen preset in the constructor and is dispatched as `await new Usecase(chosenPreset).execute()`. `UploadVideos.ts` is the core flow (see below): it instantiates the services/repository inside `execute()` and sequences them. `PrintPresetInfo.ts` prints the preset's own fields, then calls `TelegramService.getChatData`/`getSelfData` to print the live channel and bot data. `DeleteVideo.ts` picks a video with `cli.search` (each keystroke re-queries `VideosRepository.getAll(input)`; options are labelled by title), then asks a `cli.confirm` — answering no logs `Deletion cancelled.` and returns without deleting.
