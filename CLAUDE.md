...
- **`src/usecases/`** — orchestration. Every usecase is a class implementing the `Usecase` interface (`preset` field + `async execute()`), takes the chosen preset in the constructor and is dispatched as `await new Usecase(chosenPreset).execute()`. `UploadVideos.ts` is the core flow (see below): it instantiates the services/repository inside `execute()` and sequences them. `PrintPresetInfo.ts` prints the preset's own fields, then calls `TelegramService.getChatData`/`getSelfData` to print the live channel and bot data.
